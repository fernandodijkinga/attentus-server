...
def _perspicuus_rescale_with_cal(raw_score: Any, cal: dict[str, Any]) -> float | None:
    ...
    res_min = float(cal.get('res_min', 1.0))
    res_max = float(cal.get('res_max', 9.0))
    step = float(cal.get('step', 0.5))
    if raw_max <= raw_min:
        return res_min
    if res_max <= res_min:
        return res_min
    ...
    t = max(0.0, min(1.0, t))
    y = res_min + t * (res_max - res_min)
    y = round(y / step) * step
    y = max(res_min, min(res_max, y))
...
def _load_db_auth_users(db: sqlite3.Connection) -> dict[str, dict[str, Any]]:
    ...
    for r in rows:
        ...
        if not username or not pass_hash:
            continue
        raw_envs = r['envs_json']
        parsed_envs: Any = raw_envs
        if isinstance(raw_envs, str):
            txt = raw_envs.strip()
            if txt.startswith('['):
                try:
                    parsed_envs = json.loads(txt)
                except json.JSONDecodeError:
                    parsed_envs = txt
        envs = _normalize_env_access(parsed_envs)
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS perspicuus_angus_records (
        id             INTEGER PRIMARY KEY AUTOINCREMENT,
        created_at     TEXT    NOT NULL,
        farm_id        TEXT    NOT NULL,
    ...
    try:
        ...
        if nelore_cols and "birth_date" not in nelore_cols:
    ...
    try:
        angus_cols = {row[1] for row in db.execute("PRAGMA table_info(perspicuus_angus_records)")}
        if angus_cols and "lot_id" not in angus_cols:
            db.execute("ALTER TABLE perspicuus_angus_records ADD COLUMN lot_id TEXT NOT NULL DEFAULT ''")
        if angus_cols and "birth_date" not in angus_cols:
...
@app.route('/api/perspicuus-angus/media/<farm>/<day>/<filename>')
@login_required
def serve_perspicuus_angus_media(farm, day, filename):
    base = os.path.abspath(os.path.join(ANGUS_UPLOADS_DIR, ecc_safe_slug(farm), ecc_safe_slug(day)))
...
def _angus_save_one(
    ...
    if ext not in ANGUS_IMAGE_EXTS:
    ...
    folder = os.path.join(ANGUS_UPLOADS_DIR, safe_farm, safe_day)
...
def _angus_base_lists(db):
    farms = [r[0] for r in db.execute("SELECT DISTINCT farm_id FROM perspicuus_angus_records ORDER BY farm_id").fetchall()]
    animals = [r[0] for r in db.execute("SELECT DISTINCT animal_tag FROM perspicuus_angus_records ORDER BY animal_tag").fetchall()]
    stats = {
...
def perspicuus_angus_importar():
    ...
    if request.method == 'POST':
        ...
        if mode == 'single':
            ...
            if not farm_id or not date_iso or not animal_tag or not fs or not fs.filename:
                flash('Informe fazenda, data, brinco e imagem lateral.', 'error')
    ...
    q = str(request.args.get('q', '')).strip()
    records = _angus_load_rows(db, farm_filter, animal_filter, q, limit=500)
    farms, animals, stats = _angus_base_lists(db)
    return render_template(
        'perspicuus_angus_importar.html',
        records=records[:120],
        farms=farms,
        animals=animals,
...
@app.route('/perspicuus-angus/analise-individual')
@login_required
def perspicuus_angus_analise_individual():
    db = get_db()
    farm_filter = str(request.args.get('farm', '')).strip()
    animal_filter = str(request.args.get('animal', '')).strip()
    q = str(request.args.get('q', '')).strip()
    records = _angus_load_rows(db, farm_filter, animal_filter, q, limit=3000)
    farms, animals, stats = _angus_base_lists(db)
...
@app.route('/perspicuus-angus/analise-populacao')
@login_required
def perspicuus_angus_analise_populacao():
    db = get_db()
    farm_filter = str(request.args.get('farm', '')).strip()
    q = str(request.args.get('q', '')).strip()
...
def perspicuus_angus_modelos():
    ...
    if request.method == 'POST':
        ...
        if role not in roles:
            flash('Função inválida.', 'error')
            return redirect(url_for('perspicuus_angus_modelos'))
        if action == 'clear':
            _clear_perspicuus_model_slot(role)
            reset_engine()
        ...
        save_registry({role: fname})
        reset_engine()
        flash(f'Modelo Angus guardado: {fname}', 'success')
        return redirect(url_for('perspicuus_angus_modelos'))
    reg = load_registry()
    slots = []
    for role in roles:
...
@app.route('/perspicuus-angus/calibragem')
@login_required
def perspicuus_angus_calibragem():
    db = get_db()
    farm_filter = str(request.args.get('farm', '')).strip()
    cal = _perspicuus_get_calibration(db, 'angus')
...
@app.route('/api/perspicuus-angus/calibragem/apply', methods=['POST'])
@login_required
def api_perspicuus_angus_calibragem_apply():
    data = request.get_json(silent=True) or {}
    err, parsed = _validate_perspicuus_cal_payload(data, 'angus')
    if err:
    ...
    cal = _perspicuus_get_calibration(db, 'angus')
    info = _perspicuus_apply_calibration(db, 'angus', cal, farm_id=farm)
    db.commit()
    return jsonify({
        'status': 'ok',
...
@app.route('/perspicuus-nelore/calibragem')
@login_required
def perspicuus_nelore_calibragem():
    db = get_db()
    farm_filter = str(request.args.get('farm', '')).strip()
    lot_filter = str(request.args.get('lot', '')).strip()
    cal = _perspicuus_get_calibration(db, 'nelore')
...
@app.route('/api/perspicuus-nelore/calibragem/apply', methods=['POST'])
@login_required
def api_perspicuus_nelore_calibragem_apply():
    data = request.get_json(silent=True) or {}
    err, parsed = _validate_perspicuus_cal_payload(data, 'nelore')
    if err:
        return jsonify({'error': err}), 400
    farm = str(data.get('farm_id') or '').strip()
...
@app.route('/api/record/perspicuus-angus/<int:rid>', methods=['PATCH'])
@login_required
def patch_perspicuus_angus_record(rid):
    data = request.get_json(silent=True) or {}
    allowed = {'birth_date', 'sex'}
    updates = {k: str(v).strip() if v is not None else '' for k, v in data.items() if k in allowed}
    if not updates:
        return jsonify({'error': 'Nenhum campo válido'}), 400
    if 'sex' in updates:
        sx = updates['sex'].lower()
        if sx not in {'', 'm', 'f'}:
            return jsonify({'error': "sex deve ser vazio, 'm' ou 'f'"}), 400
        updates['sex'] = sx
    if 'birth_date' in updates and updates['birth_date']:
        try:
            updates['birth_date'] = ecc_parse_iso_day(updates['birth_date']) or ''
        except Exception:  # noqa: BLE001
            updates['birth_date'] = ''
    sets = ', '.join(f"{k} = ?" for k in updates)
    vals = list(updates.values()) + [rid]
    db = get_db()
    cur = db.execute(f"UPDATE perspicuus_angus_records SET {sets} WHERE id = ?", vals)
    db.commit()
    if cur.rowcount == 0:
        return jsonify({'error': 'Não encontrado'}), 404
    return jsonify({'status': 'updated'})
...
@app.route('/api/record/perspicuus-nelore/<int:rid>', methods=['PATCH'])
@login_required
def patch_perspicuus_nelore_record(rid):
    data = request.get_json(silent=True) or {}
    allowed = {'birth_date', 'sex'}
    updates = {k: str(v).strip() if v is not None else '' for k, v in data.items() if k in allowed}
    if not updates:
    ...
    vals = list(updates.values()) + [rid]
    db = get_db()
    cur = db.execute(f"UPDATE perspicuus_nelore_records SET {sets} WHERE id = ?", vals)
    db.commit()
    if cur.rowcount == 0:
        return jsonify({'error': 'Não encontrado'}), 404
    return jsonify({'status': 'updated'})
